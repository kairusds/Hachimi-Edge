...
/// Dobby handles the W^X page permission transitions and ARM64 icache flush internally.
///
/// # Safety
/// On iOS without jailbreak the process must have the `com.apple.security.cs.allow-jit`
/// entitlement embedded in the IPA for Dobby's trampoline allocation to succeed.
...
/// Uses Dobby's resolver which wraps `dlopen(NULL, …)` + `dlsym`.
...
// Vtable hooking is unused on iOS (same as Android)
